...
class _BookParkingScreenState extends State<BookParkingScreen> {
  ...
  void calculateEndTime() {
    int endHour = (_startTime.hour + _selectedHours.toInt()) % 24;
    setState(() {
    ...
    Future<double> getPricePerHour(String spotId) async {
      DocumentSnapshot spotSnapshot = await FirebaseFirestore.instance
          .collection('parking_spots').doc(spotId).get();

      if (spotSnapshot.exists) {
        return spotSnapshot['pricePerHour'] ?? 0.0;
      } else {
    ...
    Future<void> saveBookingDetails(String userId, DateTime selectedDate,
        TimeOfDay startTime, TimeOfDay endTime, double duration,
        String spotId) async {
      try {
        double pricePerHour = await getPricePerHour(spotId);
                            ...
                            TextButton(
                              onPressed: () async {
                                ...
                                if (picked != null && picked != _startTime) {
                                  setState(() {
                                    _startTime = picked;
                                    final endHour = (_startTime.hour +
                                        _selectedHours.toInt()) % 24;
                                    _endTime = TimeOfDay(hour: endHour,
